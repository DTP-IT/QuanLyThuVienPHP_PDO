<?php
	...
	class BorrowCard extends Connect
	{
		...
		public function addBorrowCard($name, $date){
			if ($date == null) {
				$sqlAdd = "INSERT INTO muon_tra(maDocGia) VALUES (:name)";
		...
		public function listBorrowCardReader($id){
			$sqlView = "SELECT muon_tra.id, muon_tra.maDocGia, muon_tra.ngayTao, doc_gia.hoTen FROM muon_tra,doc_gia WHERE muon_tra.maDocGia =doc_gia.maThe AND muon_tra.maDocGia = :id";
			$preView = $this->pdo->prepare($sqlView);
